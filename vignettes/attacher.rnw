...
You can also combine \code{tab\_cap} with \code{tab.cap}. \textbf{N.B.} you can refer to objects created in the chunk if you've set the knit options \code{eval.after} to include \code{tab.cap} (which is begin set when the \code{attacher} package is being attached).
